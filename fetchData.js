fetch("http://127.0.0.1/snmp_project/server/UDP_info.php")
  ...
  .then((body) => {
    document.getElementById("udp-chlidren").innerHTML = body;
    fetch("http://127.0.0.1/snmp_project/server/System_Info.php")
      .then((response) => response.text())
      .then((body) => {
        document.getElementById("system_children").innerHTML = body;
        fetch("http://127.0.0.1/snmp_project/server/ARP_Info.php")
          .then((response) => response.text())
          .then((body) => {
            document.getElementById("arp_children").innerHTML = body;
